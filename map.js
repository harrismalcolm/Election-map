...
var alice = createPolitician("Alice Jennings", [132,17,11]);

var jane = createPolitician("Jane Thompson", [245,141,136]);

alice.electionResults = [5, 1, 7, 2, 33, 6, 4, 2, 1, 14, 8, 3, 1, 11, 11, 0, 5, 3, 3, 3, 7, 4, 8, 9, 3, 7, 2, 2, 4, 2, 8, 3, 15, 15, 2, 12, 0, 4, 13, 1, 3, 2, 8, 21, 3, 2, 11, 1, 3, 7, 2 ];
...
alice.electionResults[9] = 1;
jane.electionResults[9] = 28;

alice.electionResults[4] = 17;
jane.electionResults[4] = 38;

alice.electionResults[43] = 11;
...
var setStateResults = function(state)
  {
      ...
      if (alice.electionResults[state] > jane.electionResults[state]){
        
          theStates[state].winner = alice;
        
      } else if(alice.electionResults[state] < jane.electionResults[state]){
    ...
    politician1.innerText = alice.name;
    
    var result1 = body.children[0].children[1];
    result1.innerText = alice.electionResults[state];
    
    var politician2 = body.children[1].children[0];
    politician2.innerText = jane.name;
    
    var result2 = body.children[1].children[1];
    result2.innerText = jane.electionResults[state];
...
alice.tallyUpTotalVotes();
jane.tallyUpTotalVotes();

console.log(alice.totalVotes);
console.log(jane.totalVotes);

var winner = ""
 
  if (alice.totalVotes < jane.totalVotes) {
     winner = jane.name;
 } else if (alice.totalVotes > jane.totalVotes){
     winner = alice.name;
 } else {
     winner = "DRAW!";
 }
...
console.log("Announcing your new US President, " + winner + "!");

var table = document.getElementById("countryResults");
var row = table.children[0].children[0];

row.children[0].innerText = alice.name;
row.children[1].innerText = alice.totalVotes;
